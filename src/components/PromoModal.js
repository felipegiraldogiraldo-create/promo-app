...
export default function PromoModal({ modal, asins, onSave, onDelete, onClose, existingPromos }) {
  ...
  const startEdit = (p) => {
    setEditingId(p.id);
    setAsin(p.asin);
    setSubtype(p.subtype || 'Lightning Deal');
  ...
  const cancelEdit = () => {
    setEditingId(null);
    setAsin(modal.asin || '');
    setSubtype('Lightning Deal');
    setDiscount('');
    setDateStart(modal.date || '');
    setDateEnd(modal.date || '');
    setConfirmed(true);
    setNote('');
  };
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      ...
      onSave({ asin, type, subtype, discount, dateStart, dateEnd, confirmed, note, editId: editingId });
    } else {
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', display:'flex',
      ...
      <div style={{ background:'#1E293B', borderRadius:12, padding:28, width:'100%', maxWidth:480,
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
          <h3 style={{ color:'#F1F5F9', fontSize:16, fontWeight:600 }}>{editingId ? 'Edit Promotion' : 'Add Promotion'}</h3>
        ...
        <div style={{ display:'grid', gap:14 }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            <button onClick={handleSave}
              style={{ flex:2, background:'#3B82F6', border:'none', color:'#fff', borderRadius:8,
